...
class PhillipsRCTest(CipherTestBase):
    cipher = PhillipsRC(CipherTestBase.ALPHABET.replace(b'j', b''), CipherTestBase.UNKNOWN_SYMBOL, CipherTestBase.UNKNOWN_SYMBOL_NUMBER)
    plaintext = b'Squares one and five are actually the same as are squares two and eight. The overall period is forty.'
    ciphertext = b'kzwlyrgfirufqarnpygplfxwlppoybrykurkgkyrkzwlyrgvcfufqgrvqaaqtcityfppvslmehmgufyvo'
